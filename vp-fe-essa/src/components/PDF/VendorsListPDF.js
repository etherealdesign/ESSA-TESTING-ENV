...
const VendorsListPDF = ({data, headerLabels}) => {
    ...
    <Page size="A4"  style={styles.page}>
      <View style={styles.table}>
        ...
        <View style={styles.tableRowHeader}>
          <Text style={[styles.tableCol, styles.colVendorCode]}>{headerLabels[0]}</Text>
        ...
        {data.map((item, idx) => (
          <View style={[idx % 2 === 0 ? styles.tableRowEven : styles.tableRowOdd,styles.row]} key={idx}>
            ...
            <Text style={[styles.tableCol, styles.colStatus]}>{item.isActive ? "Active" : "Inactive"}</Text>
          </View>
        ))}
      </View>
    </Page>
