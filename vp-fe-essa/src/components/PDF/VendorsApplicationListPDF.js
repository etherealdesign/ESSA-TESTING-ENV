...
const VendorsApplicationListPDF = ({data, headerLabels}) => {
  Font.register({
  family: 'Amiri',
  src: AmiriRegular,
});
    ...
    <Page size="A4"  style={styles.page}>
      <View style={styles.table}>
        ...
        <View style={styles.tableRowHeader}>
          ...
          <Text style={[styles.tableCol, styles.colRefNo]}>{headerLabels[1]}</Text>
        ...
        {data.map((item, idx) => (
          <View style={[idx % 2 === 0 ? styles.tableRowEven : styles.tableRowOdd,styles.row]} key={idx}>
            ...
            <Text style={[styles.tableCol, styles.colRefNo]}>{item.referenceNo}</Text>
...
export default VendorsApplicationListPDF;
